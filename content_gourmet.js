...
レベル薄:
記載がある事実（経験年数・業態・希望条件）に必ず触れた上で、詳細が分からない部分は正直に書く。型の例: 「{{経験年数}}を{{記載の業態や希望}}で重ねてこられた中身を、ご登録内容だけで判断するのは失礼だと思っています。だからこそ直接お話を伺いたいです。」誇張しない。「ご登録内容だけでは分からない」という断りだけで終わらせない。
...
- すべてのメールで、候補者情報に記載された事実への言及を最低2箇所入れる。素材の優先順位は、飲食店経験年数 → 経験業態 → 希望業態 → 希望職種 → 希望勤務地または住所のエリア → 就業区分。記載のある項目を優先順位の上位から使う。全項目が空欄の場合のみ例外とする
