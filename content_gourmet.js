...
古川の経歴は信用を補うために使います。候補者への共感を演出するために何度も使ってはいけません。本文で古川自身に触れるのはブロック6の1箇所だけです。
...
同じ候補者事実は、件名と本文冒頭（ブロック1〜2）を合わせて2回まで使用できます。本文後半で同じ内容を言い換えて繰り返してはいけません。
...
- 文末を「と思います」「と感じています」ばかりにしない。文の長さをそろえず、25文字以内の短い文を2つ以上入れる
- 意図的な誤字、口語の乱用、不自然な崩し方で人間らしさを演出しない
- 候補者の経歴を具体的に引用する（社名・店名・業務名を出す）のは件名・ブロック1・ブロック2だけ。ブロック6の2つ目の段落とクロージングの1文目では、職種レベルの語（接客・調理など）に軽く触れるだけにとどめる
- 社名・店名は候補者情報の表記どおりに書く。正式社名への言い換えや読み仮名化を勝手にしない
- 業態と店名を併記する場合は「居酒屋「味兵衛」で」のように書く。「居酒屋である味兵衛」のような「〜である」でのつなぎ方をしない
...
- 情報が少ない場合の型: 「{{確認できる事実}}というご登録内容を拝見し、ご連絡しました。」
...
- 職歴上の変化がある場合: 記載された2つの職歴を事実として示し、その間のことを直接聞きたいと書く。転職理由や現在の気持ちは推測しない
- 管理経験が具体的な場合: 記載された管理範囲や数値を示し、その役割の中で実際にどこまで担当していたのか、または今後どこまで広げたいのかを聞きたいと書く。評価語を加えない
...
- 情報が少ない場合: 確認できる事実を1つ使い、「仕事内容の詳細までは分からなかったため、これまで実際に担当されてきたことを直接お聞きしたいと思いました。」の趣旨で正直に書く。無理に褒めたり、個別に熟読したように装ったりしない
...
入社後は店舗での調理や接客から始め、スタッフの育成、シフト管理、売上や原価の管理を順に覚えていただきます。
...
- 希望職種がマネジメント系（店長/支配人、エリアマネージャー等）、または記載がない場合: 「ここまでのご経験を次はどこでどう活かしていきたいのか、一度伺ってみたいと思っています。」をそのまま使う

続けて確定文（そのまま使う）:

すぐに応募を決めていただく必要はありません。
まずは仕事内容を聞いてみたい場合はカジュアル面談、選考を希望される場合は面接として、どちらもオンラインでお話しできます。
「話だけ聞きたい」「面接希望」の一言で構いません。
少しでも気になりましたら、ご返信いただけると嬉しいです。
